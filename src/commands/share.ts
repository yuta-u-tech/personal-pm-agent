...
export async function shareCommand(targetDir: string, options: { adapter?: string; date?: string } = {}): Promise<void> {
  ...
  await mkdir(path.join(targetDir, "reports/share"), { recursive: true });
  await writeFile(path.join(targetDir, "reports/share", `${date}.md`), renderShareReport(report), "utf8");
}
